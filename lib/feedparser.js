...
class FeedReader {
  ...
  async parse() {
    const promises = await this.urls.map((url) => FeedReader.read(url))
  ...
  static read(url) {
    ...
    instance.interceptors.response.use(FeedReader.iconvInterceptor)
    ...
    return instance(url, {
      ...
      .then((response) => {
        const feedType = FeedReader.getFeedType(response.data, url)

        return FeedReader[feedType](response.data, url)
      })
      ...
      parser.parseString(xml, (error, result) => {
        ...
        const articles = entries.map((entry) => {
          if (entry['media:group']) {
            // YouTube
            return FeedReader.AtomYouTube(entry, source)
          }
